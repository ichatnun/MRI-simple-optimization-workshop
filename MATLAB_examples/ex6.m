% We have seen earlier that our system is under-determined. 
...
subplot(1,3,2), imagesc(abs(Gx(:,:,1))), colormap gray, axis image off, clim(dynamic_range), title('G*x along the x-axis')
subplot(1,3,3), imagesc(abs(Gx(:,:,2))), colormap gray, axis image off, clim(dynamic_range), title('G*x along the y-axis')
...
for iter = 1:num_iters
    ...
    grad_reg_term = computeGradientRegTerm(x,G);
...
% Compute the gradient of the regularization term: ||Gx||_1
function grad = computeGradientRegTerm(x,G)
    Gx = ...
    grad = ...
end
